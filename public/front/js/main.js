...
document.querySelectorAll('.nav-link').forEach(link => {
      link.addEventListener('mouseover', function () {
          ...
          const descId = 'desc-' + this.getAttribute('data-id');

          // Update title
          document.getElementById('overlay-title').textContent = title;

          // Update image
          document.getElementById('service-image').setAttribute('src', img);

          // Update description (with HTML from hidden div)
          const htmlDesc = document.getElementById(descId)?.innerHTML || '';
          document.getElementById('overlay-desc').innerHTML = htmlDesc;
      });
});
